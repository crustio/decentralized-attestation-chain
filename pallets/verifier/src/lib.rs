...
decl_storage! {
	trait Store for Module<T: Config> as Verifier {
		WaitingQueue get(fn waiting_queue): double_map hasher(twox_64_concat) T::BlockNumber, hasher(twox_64_concat) T::AccountId => Vec<u8>;
	}
}
...
decl_module! {
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		pub fn request_verification(origin, evidence: Vec<u8>) -> DispatchResult {
			...
			<WaitingQueue<T>>::insert(current_block_number + T::VerifyDelay::get(), who, evidence);
			Ok(())
		}
...
impl<T: Config> Module<T> {
	...
	fn resolve_evidence_and_send_unsigned_for_any_account(who: T::AccountId, evidence: Vec<u8>, block_number: T::BlockNumber) -> Result<(), &'static str> {
		// Make an external HTTP request to fetch the current price.
		// Note this call will block until response is received.
		let (pubkey, code, who) = Self::resolve_evidence(evidence).map_err(|_| "Failed to fetch price")?;
	...
	fn resolve_evidence(evidence: Vec<u8>) -> Result<(Vec<u8>, Vec<u8>, T::AccountId), http::Error> {
		...
		let (pubkey, code, who) = match Self::parse_result(body_str) {
			Some((pubkey, code, who)) => Ok((pubkey, code, who)),
			None => {
		...
		log::warn!("Got pubkey: {:?}, code: {:?}, who: {}", pubkey, code, who);
	...
	fn parse_result(result_str: &str) -> Option<(Vec<u8>, Vec<u8>, T::AccountId)> {
		let val = lite_json::parse_json(result_str);
		let rsts = val.ok().and_then(|v| match v {
			JsonValue::Object(obj) => {
				obj.into_iter().filter_map(|(k, v)| {
					match v {
